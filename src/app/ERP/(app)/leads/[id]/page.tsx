...
export default async function LeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [lead, activities, reminders, stagesResult, teamMembers, memberRow] = await Promise.all([
    ...
    supabase.from("organization_members").select("role").eq("organization_id", profile?.current_org_id).eq("user_id", user.id).maybeSingle(),
  ]);
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-4 pt-4">
          <Link href="/ERP/leads" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground">
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <Card>
              <CardContent className="py-3 flex gap-2">
                <Link href={`/ERP/leads/${lead.id}/invoices`} className="flex-1 text-center text-sm text-primary hover:underline">Invoices</Link>
                <Link href={`/ERP/leads/${lead.id}/payments`} className="flex-1 text-center text-sm text-primary hover:underline">Payments</Link>
              </CardContent>
            </Card>
          </div>
